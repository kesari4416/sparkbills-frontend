...
const empty = { name: "", phone: "", email: "", gstin: "", address: "", state: "Karnataka", state_code: "29", opening_balance: 0 };
...
export default function Suppliers() {
  ...
  const save = async () => {
    try {
      ...
      toast.success("Supplier added"); setOpen(false); setForm(empty); load();
    } catch (e) { toast.error(formatApiError(e)); }
  };
  const del = async (id) => { if (!confirm("Delete?")) return; await api.delete(`/suppliers/${id}`); load(); };
    ...
    <div className="p-6 lg:p-8" data-testid="suppliers-page">
      <div className="flex items-center justify-between mb-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="rounded-sm gap-2" data-testid="new-supplier-btn"><Plus className="w-4 h-4" /> New Supplier</Button></DialogTrigger>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button data-testid="save-supplier" className="rounded-sm" onClick={save}>Save</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
          ...
          <TableBody>
            {items.map((s) => (
              <TableRow key={s.id}>
                ...
                <TableCell className="text-right"><Button size="icon" variant="ghost" onClick={() => del(s.id)}><Trash2 className="w-3.5 h-3.5" /></Button></TableCell>
              </TableRow>
            ))}
